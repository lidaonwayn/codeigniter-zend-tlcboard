	...
	<script type="text/javascript" src="{$assets_path}js/fancybox/jquery.mousewheel-3.0.4.pack.js"></script>
	<script type="text/javascript" src="{$assets_path}js/fancybox/jquery.fancybox-1.3.4.pack.js"></script>
	{literal}
	<script type="text/javascript">
        ...
        $(function() {
           $('textarea.tinymce').tinymce({
			...
			extended_valid_elements : "hr[class|width|size|noshade],img[!src|border:0|alt|title|width|height]",
        	...
        	$('#btt_publish','#btt_preview').click(function() {
        	...
        	var num_pic=1;
        		...
        		new AjaxUpload(buttonthumb, {
        			...
        			onComplete: function(file, response){
        				...
        				if(status.success==0){
        					...
        					$str ="<img src=\""+status.urlpath+"\" alt=\"\" />120*90<br />";
        					$str +="<img src=\""+status.urlpath_thumb+" \" alt=\"\" />70*70<br /><br />";
 			...
 			$("#post_tag").tagEditor(
             {
                ...
                {if isset($post_tag)}
                 ...
                 items: [
                    {/literal}
                    {foreach name=tag item=data from=$post_tag}
                        {if $smarty.foreach.tag.last}"{$data}"
                        {else}"{$data}",
                        {/if}
                    {/foreach}{literal}],
	      {/literal}{/if}{literal}
        ...
        function showResponse(responseText, statusText, xhr, $form)  {
             ...
             if(responseText.success==0)
        	{
        		alert('เกิดข้อผิดพลาด'+responseText.error); 
                       ...
                       if(responseText.callback=="preview"){
                           $.fancybox({
                                'width': '95%',
                                'height': '95%',
                                'autoScale': true,
